...
import qdexswapFactory from '../../build/qdexswapFactory.json'
import IqdexswapPair from '../../build/IqdexswapPair.json'
import qdexswapRouter from '../../build/qdexswapRouter.json'
...
export async function factoryFixture(_: Web3Provider, [wallet]: Wallet[]): Promise<FactoryFixture> {
  const factory = await deployContract(wallet, qdexswapFactory, [wallet.address], overrides)
  return { factory }
}
...
export async function pairFixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<PairFixture> {
  ...
  const pair = new Contract(pairAddress, JSON.stringify(IqdexswapPair.abi), provider).connect(wallet)
  ...
  const WBNBPair = new Contract(WBNBPairAddress, JSON.stringify(IqdexswapPair.abi), provider).connect(wallet)
...
export async function routerFixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<RouterFixture> {
  ...
  const router = await deployContract(wallet, qdexswapRouter, [factory.address, WBNB.address], overrides)
